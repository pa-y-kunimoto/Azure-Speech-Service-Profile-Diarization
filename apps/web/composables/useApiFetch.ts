...
export function useApiFetch() {
	...
	const apiExternalUrl = $config.public.apiExternalUrl as string | undefined;
	const apiBaseUrl = $config.public.apiBaseUrl as string | undefined;

	const baseURL = import.meta.client ? apiExternalUrl || apiBaseUrl : apiBaseUrl;
	...
	function getWebSocketUrl(endpoint: string): string {
		const externalUrl = (apiExternalUrl || apiBaseUrl) || '';
